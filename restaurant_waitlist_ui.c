/************************************************************
 * Name: Jason Yeoh
 * Description: Restaurant Waiting List System - UI
 ************************************************************/
...
{
    char *input;
    char *argValues;            // holds argument values
    int ch, i;
    LIST *waitList = NULL;
    ...
    if(argc > 3) {  // EXIT if there are multiple arg entries
        fprintf(stderr, "STANDARD ERROR");
        printf("\nContains multiple command line arguments... %s \n", argv[1]);
        exit(-1);
    }
    ...
    while ((ch = getNextNWSChar ()) != EOF)
    {
        ...
        {
            doAdd(&waitList, debugMode);
        }
        else if('c' == ch)
        {
            doCallAhead(&waitList, debugMode);
        }
        else if('w' == ch)
        {
            doWaiting(&waitList, debugMode);
        }
        else if('r' == ch)
        {
            doRetrieve(&waitList, debugMode);
        }
        else if('l' == ch)
        {
            doList(&waitList, debugMode);
        }
        else if('d' == ch)
        {
            doDisplay(waitList, debugMode);
        }
        else if('\n' == ch)
        {
